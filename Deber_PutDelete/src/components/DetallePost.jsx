...
function DetallePost() {
  ...
  // Función para borrar el post (DELETE)
  const handleEliminar = async () => {
    // PREGUNTA DE SEGURIDAD:
    // Siempre preguntamos antes de borrar para evitar accidentes
    if (!window.confirm('¿Estás seguro de que deseas eliminar este post?')) {
      return; // Si dice que no, no hacemos nada y salimos
    }

    try {
      setEliminando(true); // Activamos estado de "Eliminando..."

      // PETICIÓN DELETE:
      // Usamos el método 'DELETE' para decirle al servidor que borre este post
      const respuesta = await fetch(`/api/posts/${postId}`, {
        method: 'DELETE' // <--- AQUÍ ESTÁ LA MAGIA DEL BORRADO
      });
      ...
      // REDIRECCIÓN:
      // Como el post ya no existe, enviamos al usuario a la página principal
      navigate('/');
    } catch (error) {
      console.error('Error:', error);
      alert('Error al eliminar el post: ' + error.message);
    } finally {
      setEliminando(false); // Terminamos la acción
    }
  };
